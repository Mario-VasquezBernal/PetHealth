...
const MedicalHistory = ({ petId }) => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        const response = await fetch(`${API_URL}/public/pets/${petId}?t=${Date.now()}`); // ✅ CAMBIO: cache-busting para traer siempre datos frescos
  ...
  return (
    <div className="space-y-6">
      <h3 className="font-black text-gray-800 text-xl flex items-center gap-2 px-2">
        ...
        <div key={index} className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
          ...
          <div className="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            ...
            <span className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider flex items-center gap-1.5 ${
              record.visit_type === 'Emergencia' ? 'bg-red-100 text-red-600' : 'bg-blue-100 text-blue-600'
            }`}>
          ...
          <div className="bg-blue-50/50 px-6 py-4 flex flex-col sm:flex-row gap-6 border-b border-blue-100/50">
            <div className="flex-1">
              ...
              <p className="font-bold text-blue-900 text-lg">{record.veterinarian_name || 'No especificado'}</p>
            </div>
            <div className="flex-1">
              <p className="text-xs text-blue-400 font-bold uppercase mb-1 flex items-center gap-1"><MapPin size={12} /> Clínica</p>
              <p className="font-bold text-blue-900 text-lg">{record.clinic_name || 'No especificada'}</p>
              {record.clinic_name && (
                <a
                  href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(record.clinic_name)}`}
                  target="_blank"
          ...
          <div className="p-6 space-y-5">
            ...
            {/* Diagnóstico */}
            <div>
              <h4 className="text-xs font-black text-gray-400 uppercase mb-2 flex items-center gap-1"><Stethoscope size={14} /> Diagnóstico</h4>
            ...
            {record.notes && (
              <div className="bg-yellow-50 p-4 rounded-xl border border-yellow-100">
                <h4 className="flex items-center gap-2 text-xs font-black text-yellow-700 uppercase mb-2"><StickyNote size={14} /> Notas / Hallazgos</h4>
                <p className="text-yellow-900 text-sm italic">"{record.notes}"</p>
              </div>
            )}
