...
const Sidebar = ({ user, sidebarOpen, setSidebarOpen, onNewPet }) => {
  ...
  const menuItems = [
    ...
    { icon: Building2, label: 'Top Clínicas', path: '/clinics-directory' }, // <--- NUEVO
    ...
    <>
      ...
      <aside className="hidden lg:flex lg:flex-col lg:w-72 bg-white border-r border-gray-200 fixed h-screen z-40">
        ...
        <div className="border-t-2 border-gray-200 bg-gray-50">
          <div className="p-4">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider px-4 mb-3">Configuración</p>
            <div className="space-y-1">
              <button 
                onClick={() => navigate('/profile')}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-2xl transition-all ${
                  isActive('/profile') ? 'bg-gradient-to-r from-emerald-500 to-emerald-600 text-white' : 'text-gray-700 hover:bg-emerald-50'
                }`}
              >
        ...
        <div className="lg:hidden fixed inset-0 z-50">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-72 bg-white shadow-2xl overflow-y-auto">
            <div className="p-6 border-b border-gray-100 flex justify-between">
              <LogoHeader />
              <button onClick={() => setSidebarOpen(false)}><X className="w-5 h-5 text-gray-600" /></button>
            </div>
            
            <div className="p-6 border-b border-gray-100 flex items-center gap-3">
               <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center"><User className="text-blue-600"/></div>
               <div className="min-w-0">
                 <p className="font-semibold text-gray-900 truncate">{user?.full_name || 'Usuario'}</p>
                 <p className="text-xs text-gray-500 flex items-center gap-1"><MapPin size={12}/> Cuenca, EC</p>
               </div>
            </div>
            ...
            <nav className="p-4 space-y-1">
              {menuItems.map((item) => {
                const Icon = item.icon;
                return (
                  <button key={item.path} onClick={() => { navigate(item.path); setSidebarOpen(false); }} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-gray-50 text-gray-600">
                    <Icon className="w-5 h-5" strokeWidth={2} />
                    <span className="text-sm">{item.label}</span>
                  </button>
                );
            ...
            {/* ... Resto del sidebar móvil (Botón Mascota y Configuración igual que desktop) ... */}
          </aside>
        </div>
      )}
    </>
